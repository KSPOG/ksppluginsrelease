...
public class KspBryophytaPlugin extends Plugin {
    ...
    @Inject private KspBryophytaScript script;
    @Inject private KspBryophytaOverlay overlay;
    @Inject private OverlayManager overlayManager;
    @Inject private ClientToolbar clientToolbar;
    @Inject private BryophytaEquipmentSettings equipmentSettings;
    @Inject private BryophytaEquipmentIndex equipmentIndex;
    @Inject private ItemManager itemManager;
    private final KspMuleWorkerService muleService = new KspMuleWorkerService("Bryophyta");

    private KspBryophytaEquipmentPanel equipmentPanel;
    private NavigationButton navigationButton;

    @Provides
    KspBryophytaConfig provideConfig(ConfigManager manager) { return manager.getConfig(KspBryophytaConfig.class); }
    ...
    @Subscribe public void onWidgetLoaded(WidgetLoaded event) { if (script.isRunning()) script.onWidgetLoaded(event.getGroupId()); }
    ...
    @Subscribe public void onNpcDespawned(NpcDespawned event) {
        ...
        if (event.getNpc().getId() == KspBryophytaScript.GROWTHLING_NPC_ID) script.onGrowthlingDespawned(event.getNpc().getIndex());
    ...
    @Subscribe public void onChatMessage(ChatMessage event) {
        if (!script.isRunning() || event.getType() != ChatMessageType.GAMEMESSAGE) return;
        String message = event.getMessage();
        if (message == null) return;
        String lower = message.toLowerCase();
        if (lower.contains("you climb down through the manhole")) script.confirmManholeDescent();
        if (config.shutdownAfterDeath() && lower.contains("oh dear, you are dead")) {
    ...
    private void addEquipmentPanel() {
        ...
        equipmentPanel = new KspBryophytaEquipmentPanel(equipmentSettings, equipmentIndex, itemManager, config.strategy(), script, config);
        BufferedImage source = BryophytaEquipmentAssets.loadEquipmentSlots();
        BufferedImage iconSource = source != null && source.getWidth() >= 170 && source.getHeight() >= 61 ? source.getSubimage(117, 8, 53, 53) : source;
        BufferedImage icon = resize(iconSource, 16, 16);
        navigationButton = NavigationButton.builder().tooltip("KSP Bryophyta Equipment").priority(8).icon(icon).panel(equipmentPanel).build();
        clientToolbar.addNavigation(navigationButton);
    }

    private void removeEquipmentPanel() {
        if (navigationButton != null) { clientToolbar.removeNavigation(navigationButton); navigationButton = null; }
        if (equipmentPanel != null) equipmentPanel.shutdownPanel();
        equipmentPanel = null;
    }

    private static BufferedImage resize(BufferedImage source, int width, int height) {
        if (source == null) return new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);

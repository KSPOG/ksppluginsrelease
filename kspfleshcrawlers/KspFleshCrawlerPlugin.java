...
public class KspFleshCrawlerPlugin extends Plugin {
    ...
    private final KspMuleWorkerService muleService = new KspMuleWorkerService("Flesh Crawlers");

    private Instant startedAt;
    private long startXp;

    @Provides
    KspFleshCrawlerConfig provideConfig(ConfigManager configManager) { return configManager.getConfig(KspFleshCrawlerConfig.class); }
    ...
    long getXpGained() { if (!Microbot.isLoggedIn() || startXp <= 0L) return 0L; return Math.max(0L, Microbot.getClient().getOverallExperience() - startXp); }

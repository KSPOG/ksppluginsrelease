...
public class KspHivesEvent implements BlockingEvent {
    ...
    @Override public boolean validate(){return plugin.isForestryEventEnabled(KspForestryEvent.BEEHIVE)&&Microbot.isPluginEnabled(plugin)&&Microbot.isLoggedIn()&&!Microbot.getRs2NpcCache().query().where(x->hive(x.getId())).toList().isEmpty()&&plugin.isSelectedResourceCampfireBurnable()&&Rs2Inventory.count(plugin.getSelectedResourceId())>1;}

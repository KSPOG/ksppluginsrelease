...
public class KspWillowChopperPlugin extends Plugin {
    public static final String VERSION="0.1.3"; private static final Pattern ANIMA_BARK_PATTERN=Pattern.compile("You've been awarded <col=[0-9a-f]+>(\\d+) Anima-infused bark</col>\\.");
    ...
    private volatile long lastForestryInteractionKey=Long.MIN_VALUE; private volatile KspForestryEvent currentForestryEvent=KspForestryEvent.NONE; private volatile boolean forestryRuntimeActive; private KspStrugglingSaplingEvent saplingEvent;
    ...
    public boolean isForestryRuntimeActive(){return forestryRuntimeActive;}
    public boolean isForestryEventEnabled(KspForestryEvent event){if(!forestryRuntimeActive||config==null||!config.enableForestry()||event==null||event==KspForestryEvent.NONE)return false;switch(event){case RISING_ROOTS:return config.rootEvent();case STRUGGLING_SAPLING:return config.saplingEvent();case FRIENDLY_ENTLINGS:return config.entlingsEvent();case BEEHIVE:return config.hivesEvent();case PHEASANT:return config.eggEvent();case POACHERS:return config.foxEvent();case ENCHANTMENT_RITUAL:return config.ritualEvent();case LEPRECHAUN:return config.leprechaunEvent();case FLOWERING_TREE:return config.flowersEvent();default:return false;}}
    @Provides KspWillowChopperConfig provideConfig(ConfigManager m){return m.getConfig(KspWillowChopperConfig.class);}
    @Override protected void startUp(){forestryRuntimeActive=true;completedForestryEvents.set(0);logsChopped.set(0);animaBarkGained.set(0);currentForestryEvent=KspForestryEvent.NONE;lastForestryInteractionMillis.set(0);lastForestryInteractionKey=Long.MIN_VALUE;synchronized(forestryCompletionLock){forestryCompletionTimes.clear();}muleService.start(config);overlayManager.add(overlay);if(config.enableForestry())addForestryEvents();script.run(config);}
    @Override protected void shutDown(){forestryRuntimeActive=false;removeForestryEvents();currentForestryEvent=KspForestryEvent.NONE;muleService.shutdown();script.shutdown();ritualCircles.clear();saplingIngredients.clear();overlayManager.remove(overlay);}
    private void addForestryEvents(){removeForestryEvents();if(!forestryRuntimeActive||config==null||!config.enableForestry())return;if(config.rootEvent())addForestryEvent(new KspRootEvent(this));if(config.saplingEvent()){saplingEvent=new KspStrugglingSaplingEvent(this);addForestryEvent(saplingEvent);}if(config.entlingsEvent())addForestryEvent(new KspEntlingsEvent(this));if(config.hivesEvent())addForestryEvent(new KspHivesEvent(this));if(config.eggEvent())addForestryEvent(new KspEggEvent(this));if(config.foxEvent())addForestryEvent(new KspFoxEvent(this));if(config.ritualEvent())addForestryEvent(new KspRitualEvent(this));if(config.leprechaunEvent())addForestryEvent(new KspLeprechaunEvent(this));if(config.flowersEvent())addForestryEvent(new KspFlowersEvent(this));}
